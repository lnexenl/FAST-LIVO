...
void Preprocess::pandar_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  for (int i = 0; i < pl_orig.points.size(); i++) {
      ...
      added_pt.curvature = double(pl_orig.points[i].timestamp);
      std::cout << std::fixed << std::setprecision(12)<< added_pt.curvature << std::endl;
...
void Preprocess::velodyne_handler(
  ...
  yaw_cali = yaw_cali > 300.0    ? (yaw_cali - 360.0)
             : yaw_cali < -300.0 ? (yaw_cali += 360.0)
                                 : yaw_cali;
